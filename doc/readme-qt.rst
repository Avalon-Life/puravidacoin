PuraVidaCoin-qt: Qt4 GUI for PuraVidaCoin
===============================
...
Alternatively, install Qt Creator and open the `PuraVidaCoin-qt.pro` file.

An executable named `PuraVidaCoin-qt` will be built.
...
A warning for people using the *static binary* version of PuraVidaCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of PuraVidaCoin is linked against libdb5.3.
...
installed causes PuraVidaCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.

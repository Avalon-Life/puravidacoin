    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %sPVC %s\n", FormatMoney(wtx.GetCredit(MINE_ALL)).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
bool CWallet::CreateCoinStake(uint256 &hashTx, uint32_t nOut, uint32_t nGenerationTime, uint32_t nBits, CTransaction &txNew, CKey& key)
{
	...
	int the_height = GetLastBlockIndex(pindexBest, false)->nHeight;
	if (GetLastBlockIndex(pindexBest, true)->nHeight > the_height);
		the_height = GetLastBlockIndex(pindexBest, true)->nHeight;
		
    nCredit += GetProofOfStakeReward(nCoinAge, nBits, nGenerationTime, false, the_height);

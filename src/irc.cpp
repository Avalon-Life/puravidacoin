...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("PuraVidaCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #PuraVidaCoinTEST2\r");
            Send(hSocket, "WHO #PuraVidaCoinTEST2\r");
        } else {
            // randomly join #PuraVidaCoin00-#PuraVidaCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #PuraVidaCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #PuraVidaCoin%02d\r", channel_number).c_str());
        }

...
const string strMessageMagic = "PuraVidaCoin Signed Message:\n";
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, unsigned int nBits, int64_t nTime, bool bCoinYearOnly, int nHeight)
{
	...
	if (nHeight >= FIRST_POS_BLOCK)
		nRewardCoinYear = 10 * CENT;
		
	int64_t moneysupply = pindexBest->nMoneySupply;
	int64_t maxsupply = 1000000000 * COIN;
	
	if (moneysupply > maxsupply)
    ...
    if (!IsCoinBase())
    {
        ...
        if (IsCoinStake())
        {
            if (nTime >  Checkpoints::GetLastCheckpointTime())
            {
                ...
                int64_t nCalculatedReward = GetProofOfStakeReward(nCoinAge, pindexBlock->nBits, nTime, false, pindexBlock->nHeight) - GetMinFee(1, false, GMF_BLOCK, nTxSize) + CENT;
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("PuraVidaCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in PuraVidaCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // PuraVidaCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PuraVidaCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "PuraVida Genesis Block";
        CTransaction txNew;
        txNew.nTime = 1462403944;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1462403946;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 78694; // !fTestNet ? 1575379 : 46534;
	
        block.print();

        //// debug print
        assert(block.hashMerkleRoot == uint256("0xbbfec418114c373992768d74b0ee54881cef761c0d3f607eb414641e4b7f4390"));
...
unsigned char pchMessageStart[4] = { 0xb4, 0xf5, 0xe6, 0xf2 };
